...
keyboard bounds(173, 113, 400, 100) channel("keyboard10000")
rslider bounds(181, 86, 60, 60) channel("index") range(0, 2, 0, 1, 0.001)
</Cabbage>
<CsoundSynthesizer>
...
instr	1	;CALL ftmorphi UDO AND CREATE AN AUDIO OSCILLATOR
	...
	iindex = i(gkindex);table		inote,gimap			; READ INDEX VALUE (RANGE 0 - 2) FROM INDEX MAP
	imorftab	ftgentmp	0,0,1024,10,1			; CREATE MORPHED TABLE FOR THIS NOTE ONLY
			ftmorfi		iindex,gitabs,imorftab		; CALL UDO
	a1		poscil		1,220,imorftab	; AUDIO OSCILLATOR THAT USES THE MORPHED TABLE
